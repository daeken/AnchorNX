...
namespace IpcGenerator {
	public abstract class IpcType {
		public static readonly IpcUnknownType Unknown = new();
		public static readonly IpcBoolType Bool = new();
		public static readonly IpcIntType U8 = new(8, false),
			U16 = new(16, false),
			U32 = new(32, false),
			U64 = new(64, false), 
			U128 = new(128, false);
		public static readonly IpcIntType I8 = new(8, true),
			I16 = new(16, true),
			I32 = new(32, true),
			I64 = new(64, true);
		public static readonly IpcFloatType F32 = new(32), F64 = new(64);
	...
	class Program {
		const string Root = "../AnchorNX/IpcServices/Generated/";
		...
		static IpcInterface RemoveDuplicates(IpcInterface iface) =>
			new(iface.Name, iface.Services, iface.Commands.Where((command, i) =>
		...
		static void Build(string ns, IReadOnlyDictionary<string, IpcType> types, IReadOnlyDictionary<string, IpcInterface> ifaces) {
			string GenType(IpcType type, string modifiers = null, bool inStruct = false, bool isUse = false) {
				...
				switch(type) {
					...
					case IpcBufferType(var btype, _): return $"Buffer<{GenType(btype, isUse: true)}>";
					case IpcBytesType(_) when inStruct: throw new NotSupportedException();
					case IpcBytesType(_): return $"{modifiers}byte[]";
					case IpcHandleType(HandleStyle.Copy, var htype): return $"{modifiers}{(htype == null ? "uint" : GenType(htype))}";
					...
					case IpcStructType et when inStruct || isUse: return modifiers + Rename(et.Name);
					case IpcStructType et: return "Span<" + modifiers + Rename(et.Name) + ">";
			...
			sw.WriteLine("#pragma warning disable 169, 465, 1998");
			sw.WriteLine("using System;");
			sw.WriteLine("using System.Threading.Tasks;");
			sw.WriteLine("using UltimateOrb;");
			sw.WriteLine($"namespace AnchorNX.IpcServices.{Rename(ns)} {{");
			cb++;
			...
			foreach(var (name, _iface) in ifaces.OrderBy(x => x.Value.Services.Count != 0 ? 0 : 1).ThenBy(x => x.Key)) {
				...
				cb += $"public class _Base_{Rename(name)} : IpcInterface {{";
				cb++;
				cb += $"static readonly Logger Logger = new(\"{Rename(name)}\");";
				cb += "new static Action<string> Log = Logger.Log;";
				cb += "public override async Task _Dispatch(IncomingMessage im, OutgoingMessage om) {";
				cb++;
						...
						switch(type) {
							...
							case IpcHandleType(var style, var htype):
								...
								return style == HandleStyle.Copy
									? $"im.GetCopy{targ}({copyInOffset++})"
									: $"im.GetMove{targ}({moveInOffset++})";
							case IpcIntType(var size, var signed): {
							...
							case IpcObjectType(var iname):
								...
								if(iname != null)
									harg = $"<{Rename(iname)}>";
								return $"im.GetMove{harg}({moveInOffset++})";
							case IpcPidType _: return "im.Pid";
							case IpcStructType st: return $"im.GetDataSpan<{GenType(st, isUse: true)}>({inputOffset})"; // TODO: Inc offset
					...
					string GenOutputArg(IpcType type, bool isRet = false) {
						...
						switch(type) {
							...
							case IpcStructType _:
								...
								return $"om.GetDataSpan<{GenType(type, isUse: true)}>({outputOffset})"; // TODO: Inc offset
							case IpcFloatType(var size):
								Align(type.Alignment == -1 ? size / 8 : type.Alignment);
								outputHandlers.Add($"om.SetData({outputOffset}, {vname});");
								outputOffset += size / 8;
							...
							case IpcHandleType(var style, _):
								if(style == HandleStyle.Copy)
									outputHandlers.Add($"om.Copy({copyOutOffset++}, await CreateHandle({vname}, copy: true));");
								else
									outputHandlers.Add($"om.Move({moveOutOffset++}, await CreateHandle({vname}));");
								break;
				...
				foreach(var command in iface.Commands) {
					...
					var impl = command.Outputs.Count == 0 ? $"\"Stub hit for {Rename(ns)}.{Rename(name)}.{Rename(command.Name)} [{command.CommandId}]\".Debug(Log)" : "throw new NotImplementedException()";
