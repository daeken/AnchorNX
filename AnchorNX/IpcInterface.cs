...
namespace AnchorNX {
	...
	public abstract class IpcInterface {
		...
		public unsafe uint SyncMessage(Memory<byte> ipcBuf, out bool closeHandle) {
			fixed(byte* buffer = ipcBuf.Span) {
				...
				Console.WriteLine($"Got message: {incoming.DomainCommand} {incoming.CommandId} {incoming.Type}");
