...
using UltimateOrb;
namespace AnchorNX.IpcServices.Nn.Migration.User {
	...
	public class _Base_IService : IpcInterface {
		...
		public override async Task _Dispatch(IncomingMessage im, OutgoingMessage om) {
			switch(im.CommandId) {
				...
				case 100: { // CreateServer
					var ret = CreateServer(null, im.GetCopy(0), im.GetBuffer<byte>(0x19, 0));
					om.Initialize(1, 0, 0);
					om.Move(0, await CreateHandle(ret));
					break;
				}
				case 101: { // ResumeServer
					var ret = ResumeServer(null, im.GetCopy(0));
					om.Initialize(1, 0, 0);
					om.Move(0, await CreateHandle(ret));
					break;
				}
				case 200: { // CreateClient
					var ret = CreateClient(null, im.GetCopy(0), im.GetBuffer<byte>(0x19, 0));
					om.Initialize(1, 0, 0);
					om.Move(0, await CreateHandle(ret));
					break;
				}
				case 201: { // ResumeClient
					var ret = ResumeClient(null, im.GetCopy(0));
					om.Initialize(1, 0, 0);
					om.Move(0, await CreateHandle(ret));
					break;
				}
		...
		public virtual Nn.Migration.User.IServer CreateServer(object _0, uint _1, Buffer<byte> _2) => throw new NotImplementedException();
		public virtual Nn.Migration.User.IServer ResumeServer(object _0, uint _1) => throw new NotImplementedException();
		public virtual Nn.Migration.User.IClient CreateClient(object _0, uint _1, Buffer<byte> _2) => throw new NotImplementedException();
		public virtual Nn.Migration.User.IClient ResumeClient(object _0, uint _1) => throw new NotImplementedException();
	}
	...
	public class _Base_IClient : IpcInterface {
		...
		public virtual void Abort() => "Stub hit for Nn.Migration.User.IClient.Abort [500]".Debug(Log);
	...
	public class _Base_IServer : IpcInterface {
		static readonly Logger Logger = new("IServer");
		new static Action<string> Log = Logger.Log;
		public override async Task _Dispatch(IncomingMessage im, OutgoingMessage om) {
			switch(im.CommandId) {
				case 0: { // GetUid
		...
		public virtual Nn.Migration.Detail.IAsyncContext CompleteAsync() => throw new NotImplementedException();
		public virtual void Abort() => "Stub hit for Nn.Migration.User.IServer.Abort [500]".Debug(Log);
	}
}
