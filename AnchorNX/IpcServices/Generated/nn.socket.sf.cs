...
namespace AnchorNX.IpcServices.Nn.Socket.Sf {
	...
	public class _Base_IClient : IpcInterface {
		static readonly Logger Logger = new("IClient");
		new static Action<string> Log = Logger.Log;
		public override async Task _Dispatch(IncomingMessage im, OutgoingMessage om) {
			switch(im.CommandId) {
				case 0: { // RegisterClient
					var ret = RegisterClient(im.GetDataSpan<Nn.Socket.BsdBufferConfig>(8), im.GetData<ulong>(8), im.GetData<ulong>(16), im.GetCopy(0), im.Pid);
					om.Initialize(0, 0, 4);
					om.SetData(8, ret);
					break;
				}
				...
				case 5: { // Select
					Select(im.GetData<uint>(8), im.GetDataSpan<Nn.Socket.Timeout>(12), im.GetBuffer<byte>(0x21, 0), im.GetBuffer<byte>(0x21, 1), im.GetBuffer<byte>(0x21, 2), out var _0, out var _1, im.GetBuffer<byte>(0x22, 0), im.GetBuffer<byte>(0x22, 1), im.GetBuffer<byte>(0x22, 2));
					om.Initialize(0, 0, 8);
					om.SetData(8, _0);
					om.SetData(12, _1);
					break;
		...
		public virtual uint RegisterClient(Span<Nn.Socket.BsdBufferConfig> config, ulong pid, ulong transferMemorySize, uint _3, ulong _4) => throw new NotImplementedException();
		public virtual void StartMonitoring(ulong pid, ulong _1) => "Stub hit for Nn.Socket.Sf.IClient.StartMonitoring [1]".Debug(Log);
		...
		public virtual void Select(uint nfds, Span<Nn.Socket.Timeout> timeout, Buffer<byte> readfds_in, Buffer<byte> writefds_in, Buffer<byte> errorfds_in, out int ret, out uint bsd_errno, Buffer<byte> readfds_out, Buffer<byte> writefds_out, Buffer<byte> errorfds_out) => throw new NotImplementedException();
