	...
	public partial class IFileSystemProxyForLoader {
		...
		public override IFileSystem OpenCodeFileSystem(ulong tid, Buffer<byte> content_path, Buffer<byte> code_info) {
			if(Box.DisabledTitles.Contains(tid)) {
				Console.WriteLine($"Attempting to open code file system for TID {tid:X} -- disabled!");
				throw new IpcException(0x7D402);
			}
			var fn = Encoding.ASCII.GetString(content_path.Span).Split('\0', 2)[0];
			Console.WriteLine($"Attempting to open code file system '{fn}' -- TID 0x{tid:X}");
		...
		public override void SetCurrentProcess(ulong _0, ulong _1) => "Stub hit for Nn.Fssrv.Sf.IFileSystemProxyForLoader.SetCurrentProcess [2]".Debug();
	}
}