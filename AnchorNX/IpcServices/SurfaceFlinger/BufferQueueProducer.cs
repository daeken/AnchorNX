...
namespace AnchorNX.IpcServices.Nns.Hosbinder {
	class BufferQueueProducer : IGraphicBufferProducer {
		static readonly Logger Logger = new("BufferQueueProducer");
		static readonly Action<string> Log = Logger.Log;

		readonly object _callbackLock = new();
		uint _callbackTicket;
		uint _currentCallbackTicket;

		uint _nextCallbackTicket;

		uint _stickyTransform;
		...
		public override Status SetBufferCount(int bufferCount) {
			...
			lock(Core.Lock) {
				...
				if(bufferCount > BufferSlotArray.NumBufferSlots) return Status.BadValue;

				for(var slot = 0; slot < Core.Slots.Length; slot++)
				...
				var minBufferSlots = Core.GetMinMaxBufferCountLocked(false);

				if(bufferCount < minBufferSlots) return Status.BadValue;

				var preallocatedBufferCount = GetPreallocatedBufferCountLocked();

				if(preallocatedBufferCount <= 0) {
					Core.Queue.Clear();
					Core.FreeAllBuffersLocked();
				} else if(preallocatedBufferCount < bufferCount) {
					Log("Not enough buffers. Try with more pre-allocated buffers");
		...
		public override Status DequeueBuffer(out int slot,
			...
			var returnFlags = Status.Success;

			var attachedByConsumer = false;

			lock(Core.Lock) {
				if(format == PixelFormat.Unknown) format = Core.DefaultBufferFormat;

				usage |= Core.ConsumerUsageBits;

				var status = WaitForFreeSlotThenRelock(async, out slot, out returnFlags);
				...
				var graphicBuffer = Core.Slots[slot].GraphicBuffer.Object;
			...
			if(attachedByConsumer) returnFlags |= Status.BufferNeedsReallocation;
		...
		public override Status DetachBuffer(int slot) {
			lock(Core.Lock) {
				...
				if(!Core.Slots[slot].RequestBufferCalled) {
					Log($"Slot {slot} was detached without requesting a buffer");
		...
		public override Status DetachNextBuffer(out AndroidStrongPointer<GraphicBuffer> graphicBuffer,
			out AndroidFence fence
		) {
			lock(Core.Lock) {
				...
				var nextBufferSlot = BufferSlotArray.InvalidBufferSlot;
				...
				for(var slot = 0; slot < Core.Slots.Length; slot++)
					if(Core.Slots[slot].BufferState == BufferState.Free && !Core.Slots[slot].GraphicBuffer.IsNull)
						if(nextBufferSlot == BufferSlotArray.InvalidBufferSlot ||
						   Core.Slots[slot].FrameNumber < Core.Slots[nextBufferSlot].FrameNumber)
							nextBufferSlot = slot;
		...
		public override Status AttachBuffer(out int slot, AndroidStrongPointer<GraphicBuffer> graphicBuffer) {
			lock(Core.Lock) {
				...
				var status = WaitForFreeSlotThenRelock(false, out slot, out var returnFlags);
		...
		public override Status QueueBuffer(int slot, ref QueueBufferInput input, out QueueBufferOutput output) {
			...
			lock(Core.Lock) {
				...
				var maxBufferCount = Core.GetMaxBufferCountLocked(input.Async != 0);
				...
				if(!Core.Slots[slot].RequestBufferCalled) {
					Log($"Slot {slot} was queued without requesting a buffer");

					return Status.BadValue;
				}

				input.Crop.Intersect(Core.Slots[slot].GraphicBuffer.Object.ToRect(), out var croppedRect);
				...
				Core.Slots[slot].QueueTime = TimeSpanType.FromTimeSpan(Box.Stopwatch.Elapsed);
				Core.Slots[slot].PresentationTime = TimeSpanType.Zero;

				item.AcquireCalled = Core.Slots[slot].AcquireCalled;
				item.Crop = input.Crop;
				item.Transform = input.Transform;
				item.TransformToDisplayInverse = (input.Transform & NativeWindowTransform.InverseDisplay) ==
				                                 NativeWindowTransform.InverseDisplay;
		...
		public override void CancelBuffer(int slot, ref AndroidFence fence) {
			lock(Core.Lock) {
				if(Core.IsAbandoned || slot < 0 || slot >= Core.Slots.Length ||
				   !Core.IsOwnedByProducerLocked(slot)) return;
		...
		public override Status Connect(IProducerListener listener, NativeWindowApi api, bool producerControlledByApp,
			...
			lock(Core.Lock) {
				if(Core.IsAbandoned || Core.ConsumerListener == null) return Status.NoInit;

				if(Core.ConnectedApi != NativeWindowApi.NoApi) return Status.BadValue;
		...
		public override Status Disconnect(NativeWindowApi api) {
			...
			var status = Status.BadValue;

			lock(Core.Lock) {
				Core.WaitWhileAllocatingLocked();

				if(Core.IsAbandoned) return Status.Success;
		...
		int GetPreallocatedBufferCountLocked() {
			var bufferCount = 0;

			for(var i = 0; i < Core.Slots.Length; i++)
		...
		public override Status SetPreallocatedBuffer(int slot, AndroidStrongPointer<GraphicBuffer> graphicBuffer) {
			if(slot < 0 || slot >= Core.Slots.Length) return Status.BadValue;
		...
		Status WaitForFreeSlotThenRelock(bool async, out int freeSlot, out Status returnStatus) {
			var tryAgain = true;
			...
			while(tryAgain) {
				...
				var maxBufferCount = Core.GetMaxBufferCountLocked(async);
				...
				if(maxBufferCount < Core.MaxBufferCountCached)
					for(var slot = maxBufferCount; slot < Core.MaxBufferCountCached; slot++)
						if(Core.Slots[slot].BufferState == BufferState.Free && !Core.Slots[slot].GraphicBuffer.IsNull &&
				...
				for(var slot = 0; slot < maxBufferCount; slot++)
					switch(Core.Slots[slot].BufferState) {
						...
						case BufferState.Free:
							if(freeSlot == BufferSlotArray.InvalidBufferSlot ||
							   Core.Slots[slot].FrameNumber < Core.Slots[freeSlot].FrameNumber) freeSlot = slot;
							break;
					}

				// The producer SHOULD call SetBufferCount otherwise it's not allowed to dequeue multiple buffers.
				if(Core.OverrideMaxBufferCount == 0 && dequeuedCount > 0) return Status.InvalidOperation;

				if(Core.BufferHasBeenQueued) {
					var newUndequeuedCount = maxBufferCount - (dequeuedCount + 1);
					var minUndequeuedCount = Core.GetMinUndequeuedBufferCountLocked(async);

					if(newUndequeuedCount < minUndequeuedCount) {
						Log(
							$"Min undequeued buffer count ({minUndequeuedCount}) exceeded (dequeued = {dequeuedCount} undequeued = {newUndequeuedCount})");

						return Status.InvalidOperation;
					}
				}

				var tooManyBuffers = Core.Queue.Count > maxBufferCount;
		...
		protected override uint GetWaitBufferFreeEvent() {
		...
		public override Status GetBufferHistory(int bufferHistoryCount, out Span<BufferInfo> bufferInfos) {
			...
			lock(Core.Lock) {
				...
				var result = new BufferInfo[bufferHistoryCount];

				var position = Core.BufferHistoryPosition;
