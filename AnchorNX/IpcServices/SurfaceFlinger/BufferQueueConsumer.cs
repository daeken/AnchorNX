...
namespace AnchorNX.IpcServices.Nns.Hosbinder {
	class BufferQueueConsumer {
		static readonly Logger Logger = new("BufferQueueConsumer");
		static readonly Action<string> Log = Logger.Log;

		public BufferQueueConsumer(BufferQueueCore core) => Core = core;

		public BufferQueueCore Core { get; }

		public Status AcquireBuffer(out BufferItem bufferItem, ulong expectedPresent) {
			lock(Core.Lock) {
				var numAcquiredBuffers = 0;

				for(var i = 0; i < Core.MaxBufferCountCached; i++)
				...
				if(Core.StillTracking(ref bufferItem)) {
					...
					var targetFrameNumber = Core.Slots[bufferItem.Slot].FrameNumber;

					for(var i = 0; i < Core.BufferHistory.Length; i++)
				...
				if(bufferItem.AcquireCalled) bufferItem.GraphicBuffer.Reset();
		...
		public Status DetachBuffer(int slot) {
			lock(Core.Lock) {
				...
				if(slot < 0 || slot >= Core.Slots.Length || !Core.IsOwnedByConsumerLocked(slot)) return Status.BadValue;
		...
		public Status AttachBuffer(out int slot, ref AndroidStrongPointer<GraphicBuffer> graphicBuffer) {
			lock(Core.Lock) {
				...
				var freeSlot = BufferSlotArray.InvalidBufferSlot;
				...
				for(var i = 0; i < Core.Slots.Length; i++)
					if(Core.Slots[i].BufferState == BufferState.Acquired)
						...
						if(freeSlot == BufferSlotArray.InvalidBufferSlot ||
						   Core.Slots[i].FrameNumber < Core.Slots[freeSlot].FrameNumber)
							freeSlot = i;
		...
		public Status ReleaseBuffer(int slot, ulong frameNumber, ref AndroidFence fence) {
			...
			lock(Core.Lock) {
				if(Core.Slots[slot].FrameNumber != frameNumber) return Status.StaleBufferSlot;
		...
		public Status Connect(IConsumerListener consumerListener, bool controlledByApp) {
			if(consumerListener == null) return Status.BadValue;
		...
		public Status Disconnect() {
			lock(Core.Lock) {
				if(!Core.IsConsumerConnectedLocked()) return Status.BadValue;
		...
		public Status GetReleasedBuffers(out ulong slotMask) {
			...
			lock(Core.Lock) {
				if(Core.IsAbandoned) return Status.BadValue;

				for(var slot = 0; slot < Core.Slots.Length; slot++)
					if(!Core.Slots[slot].AcquireCalled)
						slotMask |= 1UL << slot;

				for(var i = 0; i < Core.Queue.Count; i++)
		...
		public Status SetDefaultBufferSize(uint width, uint height) {
			if(width == 0 || height == 0) return Status.BadValue;
		...
		public Status SetDefaultMaxBufferCount(int bufferMaxCount) {
			lock(Core.Lock) return Core.SetDefaultMaxBufferCountLocked(bufferMaxCount);
		}

		public Status DisableAsyncBuffer() {
			lock(Core.Lock) {
				if(Core.IsConsumerConnectedLocked()) return Status.InvalidOperation;
		...
		public Status SetMaxAcquiredBufferCount(int maxAcquiredBufferCount) {
			...
			lock(Core.Lock) {
				if(Core.IsProducerConnectedLocked()) return Status.InvalidOperation;
		...
		public Status SetDefaultBufferFormat(PixelFormat defaultFormat) {
			lock(Core.Lock) Core.DefaultBufferFormat = defaultFormat;

			return Status.Success;
		}

		public Status SetConsumerUsageBits(uint usage) {
			lock(Core.Lock) Core.ConsumerUsageBits = usage;

			return Status.Success;
		}

		public Status SetTransformHint(NativeWindowTransform transformHint) {
			lock(Core.Lock) Core.TransformHint = transformHint;
